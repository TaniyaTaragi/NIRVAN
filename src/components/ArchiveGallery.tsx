...
const ARCHIVE_IMAGES = [
  { src: '/assets/nirvan-3d-bg.jpg', label: 'NIRVAN // FESTIVAL IDENTITY' },
  { src: '/assets/hackathon-3d-bg.jpg', label: 'HACKATHON // BUILD FLOOR' },
  { src: '/assets/esports-3d-bg.jpg', label: 'ESPORTS // ARENA NIGHT' },
  { src: '/assets/ctf-3d-bg.jpg', label: 'CTF // CYBER SIEGE' },
  { src: '/assets/quest-3d-bg.jpg', label: 'TREASURE HUNT // QUEST' },
  { src: '/assets/workshop-3d-bg.jpg', label: 'WORKSHOPS // MASTERCLASS' },
];
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 mb-12 pb-6 border-b border-zinc-300">
        <div>
          <div className="inline-flex items-center gap-2 text-xs font-mono-code uppercase tracking-widest text-zinc-500 mb-3">
            ...
            <span>ARCHIVE // NIRVAN EDITIONS</span>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-5">
        {ARCHIVE_IMAGES.map((image, index) => (
          <figure key={image.src} className={`group relative overflow-hidden bg-zinc-200 ${index === 0 ? 'col-span-2 md:col-span-1' : ''}`}>
            <img src={image.src} alt={image.label} className="aspect-[4/3] w-full object-cover grayscale contrast-110 transition duration-700 group-hover:scale-105 group-hover:grayscale-0" />
